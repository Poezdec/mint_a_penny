...
class Help:
    def check_status_tx(self, tx_hash):
        logger.info(
            f'{self.address} - жду подтверждения транзакции - https://basescan.org/tx/{self.w3.to_hex(tx_hash)}...')
...
class Zerion(Help):
    def __init__(self, key):
        self.privatekey = key
        self.w3 = Web3(Web3.HTTPProvider(random.choice(['https://base.publicnode.com', 'https://1rpc.io/base', 'https://rpc.ankr.com/base'])))
        self.account = self.w3.eth.account.from_key(self.privatekey)
        self.address = self.account.address
        self.dna_address = Web3.to_checksum_address('0xb3da098a7251a647892203e0c256b4398d131a54')

    def mint(self):
        logger.info(f'{self.address} - начинаю минт Mint A Penny...')
        while True:
            # check gas
            gas = self.check_gas()
            if gas < gwei:
                try:
                    ...
                    if status == 1:
                        ...
                        logger.success(
                            f'{self.address} - успешно заминтил Mint A Penny {id} - https://basescan.org/tx/{self.w3.to_hex(hash_)}...')
...
def main():
    ...
    print(f'\n{" " * 32}автор - https://t.me/poezdec{" " * 32}\n')
    logger.info(f'Начинаю работу на {len(keys)} кошельков...')
    for key in keys:
        claim = Zerion(key)
        res = claim.mint()
        write_to_csv(key, *res)
    logger.success('Успешный mint...')
    print(f'\n{" " * 32}автор - https://t.me/poezdec{" " * 32}\n')
    print(f'\n{" " * 32}donate - EVM 0x23d8B6a378fAF15CE9f012dAE49097a4eec2E564{" " * 32}\n')
